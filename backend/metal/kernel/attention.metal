...
kernel void attention_prepare(
        device half* q [[buffer(0)]], device half* k [[buffer(1)]],
        device const half* qg [[buffer(2)]], device const half* raw_k [[buffer(3)]],
        device const float* q_norm [[buffer(4)]], device const float* k_norm [[buffer(5)]],
        device const half2* rope [[buffer(6)]], device const uint* positions [[buffer(7)]],
    ...
    bool query = head < Q_HEADS; uint local_head = query ? head : head - Q_HEADS;
    uint source_base = query ? (row * Q_HEADS + local_head) * HEAD_DIM * 2
                             : (row * KV_HEADS + local_head) * HEAD_DIM;
    uint output_base = query ? (row * Q_HEADS + local_head) * HEAD_DIM
                             : (row * KV_HEADS + local_head) * HEAD_DIM;
    ...
    if (!simd_lane) partials[simd_index] = sum;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (!lane) {
        float total = 0.0f; for (uint i = 0; i < 8; ++i) total += partials[i];
...
kernel void attention_scan(
        device float* partials [[buffer(0)]], device const half* q [[buffer(1)]],
    ...
    if (simd_index == 0 && simd_lane == 0) {
        partials[partial + HEAD_DIM] = attention_max; partials[partial + HEAD_DIM + 1] = attention_norm;
    }
}

[[max_total_threads_per_threadgroup(128)]]
kernel void attention_reduce(
        device half* output [[buffer(0)]], device const float* partials [[buffer(1)]],
        device const half* qg [[buffer(2)]], constant uint& rows [[buffer(3)]],
        constant uint& splits [[buffer(4)]], uint simd_lane [[thread_index_in_simdgroup]],
        uint simd_index [[simdgroup_index_in_threadgroup]], uint3 group [[threadgroup_position_in_grid]]) {
    ...
    if (simd_index == 0 && simd_lane == 0) {
        ...
        for (uint split = 1; split < splits; ++split) attention_max = max(attention_max, split_max[split]);
        attention_norm = 0.0f;
    ...
    for (uint dim = simd_index * 32 + simd_lane; dim < HEAD_DIM; dim += 128) {
        ...
        output[(row * Q_HEADS + q_head) * HEAD_DIM + dim] = half((numerator / attention_norm) /
                                                                 (1.0f + exp(-gate)));
    }
}
