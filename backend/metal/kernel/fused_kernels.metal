...
kernel void mlp_gate_up_q4_k_decode(device half* y [[buffer(0)]], device const half* x [[buffer(1)]], device const uchar* wg [[buffer(2)]],
                                    device const uchar* wu [[buffer(3)]], constant long& S [[buffer(4)]], ushort lane [[thread_index_in_simdgroup]],
...
static inline __attribute__((always_inline)) void q5k_accumulate(device const uchar* weights, uint nb, uint ib, uint row, ushort q_offset, ushort l0,
  ...
  sumf += float(dh[0]) * (sc8[0] * (acc1[0] + 16.0f * acc2[0]) + sc8[1] * (acc1[1] / 16.0f + 16.0f * acc2[1]) + sc8[4] * (acc1[2] + 16.0f * acc2[2]) +
                          sc8[5] * (acc1[3] / 16.0f + 16.0f * acc2[3])) -
          float(dh[1]) * dot(sumy, float4(sc8[2], sc8[3], sc8[6], sc8[7]));
}

[[max_total_threads_per_threadgroup(128)]]
kernel void mlp_gate_up_q5_k_decode(device half* y [[buffer(0)]], device const half* x [[buffer(1)]], device const uchar* wg [[buffer(2)]],
                                    device const uchar* wu [[buffer(3)]], constant long& S [[buffer(4)]], ushort lane [[thread_index_in_simdgroup]],
                                    ushort simd_group [[simdgroup_index_in_threadgroup]], uint3 group [[threadgroup_position_in_grid]]) {
...
static inline __attribute__((always_inline)) void iq4xs_accumulate(device const uchar* weights, uint nb, uint ib, uint first_row, ushort part,
  ...
  for (ushort row = 0; row < 2; ++row) {
    ...
    float4 acc1 = 0.0f, acc2 = 0.0f;
    aux[0] = q4[0] & 0x0f0f0f0f;
    aux[1] = (q4[0] >> 4) & 0x0f0f0f0f;
    ...
    aux[0] = q4[1] & 0x0f0f0f0f;
    aux[1] = (q4[1] >> 4) & 0x0f0f0f0f;
    ...
    int ls = int((weights[o + 4 + part / 2] >> (4 * (part % 2))) & 15) | int(((sh >> (2 * part)) & 3) << 4);
...
[[max_total_threads_per_threadgroup(128)]]
kernel void mlp_gate_up_iq4_xs_decode(device half* y [[buffer(0)]], device const half* x [[buffer(1)]], device const uchar* wg [[buffer(2)]],
                                      device const uchar* wu [[buffer(3)]], constant long& S [[buffer(4)]], ushort lane [[thread_index_in_simdgroup]],
                                      ushort simd_group [[simdgroup_index_in_threadgroup]], uint3 group [[threadgroup_position_in_grid]]) {
  uint nb = MLP_K / 256, first_row = (group.x * 4 + simd_group) * 2;
  ushort ix = lane / 16, it = lane % 16, part = it / 2, il = it % 2;
  threadgroup float lookup[16];
  if (simd_group == 0 && lane < 16)
    lookup[lane] = iq4nl[lane];
