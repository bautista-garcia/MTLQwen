...
class _Info(ctypes.Structure):
  _fields_ = ([(name, ctypes.c_uint64)
               for name in ("parameters", "weight_bytes", "mapped_bytes", "drafted", "accepted")] + [("valid", ctypes.c_uint32)])
...
class Sequence:
  ...
  @property
  def length(self):
    return _snapshot(self.engine.handle, self.handle).valid

  def speculative_counters(self):
    info = _snapshot(self.engine.handle, self.handle)
    return {
      "drafted_tokens": info.drafted,
...
class InferenceEngine:
  ...
  def __init__(self,
               ...
               *,
               drafter="none",
               draft_weights: str | Path | None = None,
               max_context=65536):
  ...
  def mapped_bytes(self):
    return _snapshot(self.handle).mapped_bytes
