...
function renderMarkdown(text) {
  ...
  let source = String(text || "").replace(/```[\s\S]*?```|`[^`\n]+`/g, (value) => {
    code.push(value);
    return `INFENGCODE${code.length - 1}TOKEN`;
  });
  source = source
    .replace(/\$\$([\s\S]+?)\$\$/g, (_, tex) => {
      math.push(renderMath(tex.trim(), true));
      return `INFENGMATH${math.length - 1}TOKEN`;
    })
    .replace(/(^|[^\\$])\$([^\n$]+?)\$(?!\$)/g, (_, prefix, tex) => {
      math.push(renderMath(tex.trim(), false));
      return `${prefix}INFENGMATH${math.length - 1}TOKEN`;
    });
...
function renderServer() {
  ...
  if (serverStatus.loaded) {
    ...
    label.textContent = "MTLQwen · online";
    const drafter = serverStatus.drafter === "none" ? "target only" : `${serverStatus.drafter.toUpperCase()} ready`;
    detail.textContent = `${serverStatus.active_sessions || 0} active · ${drafter}`;
    load.textContent = "Ready";
    load.disabled = true;
  } else if (serverStatus.loading) {
    dot.classList.add("loading");
    label.textContent = "Loading model";
    detail.textContent = "One shared instance";
    load.textContent = "Loading";
    load.disabled = true;
  } else if (serverStatus.error) {
    dot.classList.add("error");
    label.textContent = "Load failed";
    detail.textContent = serverStatus.error;
    load.textContent = "Retry";
    load.disabled = false;
  } else {
    label.textContent = "Model offline";
    detail.textContent = "Metal runtime ready";
  ...
  if (!chooser.dataset.ready || serverStatus.loaded || serverStatus.loading) {
    chooser.value = serverStatus.drafter || "none";
...
function renderSidebar() {
  ...
  for (const session of ordered) {
    ...
    if (session.generating) {
      ...
      sub.innerHTML = `<i class="mini-dot"></i>${session.metrics?.tps ? `${session.metrics.tps} tok/s` : "generating"}`;
    } else
      sub.textContent =
        session.sequence_id === null
          ? "Not allocated"
          : `SEQ ${String(session.sequence_id).padStart(2, "0")} · ${session.message_count || session.messages?.length || 0} messages`;
...
function renderTop() {
  ...
  ui.meta.textContent =
    session?.sequence_id === null || session?.sequence_id === undefined
      ? "No sequence allocated"
      : `Sequence ${session.sequence_id} · ${formatBytes(session.mapped_kv_bytes)} mapped`;
  ui.tps.textContent = formatMetric(metrics.tps, 1);
  ui.ttft.textContent = formatMetric(metrics.ttft_ms);
  ui.tokens.textContent = metrics.generated_tokens || 0;
  ui.context.textContent = metrics.context_tokens || 0;
  const generating = Boolean(session?.generating),
    stopping = Boolean(session?.stopping);
  const acceptance =
    metrics.acceptance_rate === null || metrics.acceptance_rate === undefined ? "" : ` · ${Math.round(metrics.acceptance_rate * 100)}% accepted`;
  ui.generation.textContent = stopping
    ? "Stopping after current block…"
    : generating
      ? `Generating${metrics.tps ? ` · ${metrics.tps} tok/s` : "…"}${acceptance}`
      : "Ready";
...
function messageNode(message, index) {
  ...
  if (message.thinking) {
    ...
    thought.open = Boolean(message.streaming);
    const summary = document.createElement("summary");
    summary.textContent = message.streaming ? "Thinking" : "Reasoning";
  ...
  response.className = `response-content${message.streaming ? " streaming" : ""}${message.error ? " error-text" : ""}`;
  ...
  if (!message.streaming) enhance(response);
  if (message.streaming) {
    const cursor = document.createElement("span");
    cursor.className = "cursor";
    body.append(cursor);
  }
  const stats = messageStats(message.metrics, message.cancelled);
...
function appendStreamText(element, text) {
  ...
  element.firstChild.appendData(text.slice(rendered));
  element.dataset.rendered = text.length;
}
...
function patchStreamingMessage(root, message) {
  ...
  if (Number(response.dataset.rendered || 0) !== message.content.length) {
    response.innerHTML = renderMarkdown(message.content);
    response.dataset.rendered = message.content.length;
  }
...
function renderMessages() {
  ...
  if (!session?.messages?.length) {
    ...
    welcome.innerHTML =
      '<div class="welcome-mark">MQ</div>' +
      "<h1>How can MTLQwen help?</h1>" +
      "<p>Metal-native Qwen 3.5 inference with fast, private, independent contexts.</p>" +
      '<div class="welcome-tags"><span>On device</span><span>Live tok/s</span><span>Speculative decoding</span></div>';
...
function refreshMessage(session, index) {
  ...
  session.renderFrame = requestAnimationFrame(() => {
    ...
    const message = session.messages[index],
      old = ui.messages.querySelector(`[data-index="${index}"]`);
...
async function selectSession(id) {
  ...
  if (current) current.draft = ui.input.value;
  activeId = id;
  await loadSession(id);
  if (activeId !== id) return;
  ui.input.value = sessions.get(id).draft || "";
  resizeInput();
  renderSidebar();
  renderTop();
  renderMessages();
...
async function createSession() {
  try {
    const session = mergeSession(await api("/api/sessions", { method: "POST" }));
...
function applyStreamEvent(session, message, data) {
  ...
  if (data.done) {
    ...
    message.cancelled = Boolean(data.cancelled);
  }
}
...
function createStreamPainter(session, message, index) {
  ...
  const schedule = (delay) => {
    if (timer === null) timer = setTimeout(drain, delay);
  };
  const drain = () => {
    timer = null;
    ...
    if (!data) return;
    applyStreamEvent(session, message, data);
    refreshMessage(session, index);
...
async function pollStatus() {
  try {
    ...
    serverStatus = { error: "Server unavailable", max_sessions: 8 };
    renderServer();
  }
}
...
ui.form.addEventListener("submit", async (event) => {
  ...
  if (!serverStatus.loaded && !serverStatus.loading) {
    const drafter = await chooseDrafter();
    if (!drafter) return;
    try {
      await startModelLoad(drafter);
    } catch (error) {
      notify(error.message);
      return;
    }
  ...
  renderSidebar();
  renderTop();
  streamChat(session, text, assistant, session.messages.length - 1);
});
...
$("#load-btn").onclick = async () => {
  try {
    await startModelLoad($("#model-drafter").value);
  } catch (error) {
    notify(error.message);
  }
};
...
ui.settingsScrim.onclick = closeSettings;
...
try {
  ...
  settingIds.forEach((id) => {
    if (saved[id] !== undefined) $(`#${id}`)[$(`#${id}`).type === "checkbox" ? "checked" : "value"] = saved[id];
  });
} catch {
